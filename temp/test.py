...
entry_weight_device_name_label = tk.Label(middle_left_weight_frame_left_1_frame, text="Device ID", bg="#f4f4fe", bd=0, font=(font_name, 16, "bold"))
entry_weight_device_name_label.grid(row=0, column=0, padx=5, pady=0, sticky='ew')
entry_weight_device_name_entry = tk.Entry(middle_left_weight_frame_left_1_frame, font=(font_name, 18), bd=2, textvariable=entry_weight_device_name_var)
...
entry_weight_operator_id_label = tk.Label(middle_left_weight_frame_left_1_frame, text="Operator ID", bg="#f4f4fe", bd=0, font=(font_name, 16, "bold"))
...
def weight_frame_com_port_insert_data():
    ...
    if "COM" in get_registry_value("COM1", ""):
        ...
        ser = serial.Serial(selected_com1.get(), baudrate=9600, timeout=1)
        try:
            while True:
                if ser.is_open:
...
database_test_connection_button_icon = ImageTk.PhotoImage(Image.open("icon/test_db.png").resize((146, 38)))
database_test_connection_button_hover_icon = ImageTk.PhotoImage(Image.open("icon/test_db_hover.png").resize((146, 38)))
...
threading.Thread(target=database_test_connection, args=(), daemon=True).start()
...
def check_runcard_correction(runcard):
    ...
    if len(runcard) > 0:
        ...
        threading.Thread(target=show_error_message, args=("Mày chưa nhập Runcard", 0, 3000), daemon=True).start()
...
middle_right_runcard_frame_col2_row3_col2 = None
def runcard_fill_in_entry(id):
    global middle_right_runcard_frame_col2_row3_col2
    # print(get_registry_value("is_current_entry", "weight"))
    if get_registry_value("is_current_entry", "weight") == "weight":
        entry_thickness_runcard_id_entry.delete(0, tk.END)
        entry_weight_runcard_id_entry.delete(0, tk.END)
        entry_weight_runcard_id_entry.insert(0, id)

    else:
        entry_weight_runcard_id_entry.delete(0, tk.END)
        entry_thickness_runcard_id_entry.delete(0, tk.END)
        entry_thickness_runcard_id_entry.insert(0, id)
    if middle_right_runcard_frame_col2_row3_col2:
        middle_right_runcard_frame_col2_row3_col2.destroy()

    barcode_class = barcode.get_barcode_class('code128')
    barcode_png = barcode_class(id, writer=ImageWriter())
    buffer = BytesIO()
    barcode_png.write(buffer)

    buffer.seek(0)
    pil_image = Image.open(buffer)
    pil_image = pil_image.resize((360, 200), Image.ANTIALIAS)
    barcode_image = ImageTk.PhotoImage(pil_image)



    middle_right_runcard_frame_col2_row3_col1 = tk.Frame(middle_right_runcard_frame_col2_row3, bg='white')
    middle_right_runcard_frame_col2_row3_col1.place(x=0, y=0, width=72, height=280)


    middle_right_runcard_frame_col2_row3_col2 = tk.Frame(middle_right_runcard_frame_col2_row3, bg='white')
    middle_right_runcard_frame_col2_row3_col2.place(x=72, y=0, width=288, height=280)

    middle_right_runcard_frame_col2_row3_col2_row1 = tk.Frame(middle_right_runcard_frame_col2_row3_col2, bg='white')
    middle_right_runcard_frame_col2_row3_col2_row1.place(x=0, y=0, width=288, height=40)

    middle_right_runcard_frame_col2_row3_col2_row2 = tk.Frame(middle_right_runcard_frame_col2_row3_col2, bg='white')
    middle_right_runcard_frame_col2_row3_col2_row2.place(x=0, y=40, width=288, height=100)

    middle_right_runcard_frame_col2_row3_col2_row3 = tk.Frame(middle_right_runcard_frame_col2_row3_col2, bg='white')
    middle_right_runcard_frame_col2_row3_col2_row3.place(x=5, y=140, width=288, height=40)

    middle_right_runcard_frame_col2_row3_col2_row3_label = tk.Label(middle_right_runcard_frame_col2_row3_col2_row3, text=f"{id}", bg="white", bd=0, font=(font_name, 16))
    middle_right_runcard_frame_col2_row3_col2_row3_label.pack(expand=True)

    middle_right_runcard_frame_col2_row3_col3 = tk.Frame(middle_right_runcard_frame_col2_row3, bg='white')
    middle_right_runcard_frame_col2_row3_col3.place(x=360, y=0, width=72, height=280)


    barcode_label = tk.Label(middle_right_runcard_frame_col2_row3_col2_row2, image=barcode_image, width=288, height=83)
...
def thong_tin_san_xuat(thongtin_list):
    # print(thongtin_list[1])
    middle_right_runcard_frame_col2_row0_24_label.config(text=thongtin_list[1])




def the_quy_trinh_san_xuat(machine, line, time, wo):
    def insert_data():
        try:
            if int(wo) != 0:
                sql = """SELECT rc.id, rc.WorkOrderId, wo.PartNo, wo.CustomerCode,
                        wo.CustomerName, wo.ProductItem, wo.AQL, au.Name, rc.WorkCenterTypeName, rc.MachineName, rc.LineName
                        ...
                        WHERE rc.MachineName = ? AND rc.LineName = ?
                            AND ((rc.Period > 5 AND rc.InspectionDate = CAST(GETDATE() AS DATE))
                            OR (rc.Period <= 5 AND rc.InspectionDate = DATEADD(DAY, 1, CAST(GETDATE() AS DATE))))
                            AND CAST(rc.Period AS INT) = ? AND wo.StartDate IS NOT NULL
                            AND wo.Id = ?
                        GROUP BY 
                            rc.id, rc.WorkOrderId, wo.PartNo, wo.CustomerCode, wo.CustomerName, wo.ProductItem, wo.AQL, au.Name, rc.WorkCenterTypeName, rc.MachineName, rc.LineName"""
                conn_str = (
                    f'DRIVER={{SQL Server}};'
                    f'SERVER={server_ip.get()};'
                    f'DATABASE={db_name.get()};'
                ...
                params = [machine, line, time, wo]
                new_sql = sql.replace("?", "{}").format(*params)
                # print(new_sql)
                # print(f"===>{server_ip.get()}")
                with pyodbc.connect(conn_str) as conn:
                    cursor = conn.cursor()
                    cursor.execute(sql, (machine, line, time, wo))
                    result = cursor.fetchall()
                    if result:
                        thong_tin = [f"{(datetime.datetime.today() - datetime.timedelta(hours=5) + datetime.timedelta(minutes=39)).date()}",
                                    result[0][2], result[0][8], result[0][3]]
                        thong_tin_san_xuat(thong_tin)
            else:
                thong_tin = [f"{(datetime.datetime.today() - datetime.timedelta(hours=5) + datetime.timedelta(minutes=39)).date()}",
                            '', '', '']
                thong_tin_san_xuat(thong_tin)
            ...
            thong_tin = [
                f"{(datetime.datetime.today() - datetime.timedelta(hours=5) + datetime.timedelta(minutes=39)).date()}",
                '', '', '']
            thong_tin_san_xuat(thong_tin)
            threading.Thread(target=show_error_message, args=(f"{e}", 0, 3000), daemon=True).start()
    threading.Thread(target=insert_data, daemon=True).start()
...
selected_wo_value = None
current_runcard_id = ''
def manage_wo_buttons(machine, line, time, wo_list):
    global selected_wo_button, selected_wo_value, middle_right_runcard_frame_col2_row1_middle_frame, current_runcard_id
    selected_wo_button = None
    selected_wo_value = None

    for widget in middle_right_runcard_frame_col2_row1.winfo_children():
        widget.destroy()

    if not wo_list:
        current_runcard_id = ''

    middle_width = int(len(wo_list) * 120)
    side_width = int((432 - middle_width) / 2)

    tk.Frame(middle_right_runcard_frame_col2_row1, bg='white').place(x=0, y=0, width=side_width, height=40)

    middle_right_runcard_frame_col2_row1_middle_frame = tk.Frame(middle_right_runcard_frame_col2_row1, bg='white')
    middle_right_runcard_frame_col2_row1_middle_frame.place(x=side_width, y=0, width=middle_width, height=40)

    tk.Frame(middle_right_runcard_frame_col2_row1, bg='white').place(x=side_width + middle_width, y=0, width=side_width, height=40)
    wo_button_dict = {}
    ...
    def wo_button_clicked(wo_value, btn):
        ...
        if selected_wo_button:
            try:
                selected_wo_button.config(bg="white" if len(wo_list) == 1 else "#007bff")
            except tk.TclError:
                selected_wo_button = None

        btn.config(bg="white" if len(wo_list) == 1 else "#E84A41", fg="white")
        selected_wo_button = btn
        selected_wo_value = wo_value
        current_runcard_id = selected_wo_value
        if get_registry_value("is_runcard_open", "0"):
            runcard_fill_in_entry(selected_wo_value)
            the_quy_trinh_san_xuat(machine, line, time, wo[1])
        # print(f"Selected WO: {selected_wo_value}")
    ...
    if len(wo_list) > 0:
        for index, wo in enumerate(wo_list):
            frame = tk.Frame(middle_right_runcard_frame_col2_row1_middle_frame, bg='white', width=0, height=30)
            frame.place(x=index * button_width, y=0, width=button_width, height=30)

            button = tk.Button(frame, text=wo[1], width=int(button_width/10), height=1, relief="flat", bg="#007bff", fg="white", font=("Arial", 11))
            button.config(command=lambda p=wo[0], b=button: wo_button_clicked(p, b))
            button.place(x=0, y=0, relwidth=1.0, relheight=1.0)
            wo_button_dict[wo[0]] = button
            if first_button is None:
                first_button = (wo[0], button)
        if first_button:
            wo_button_clicked(first_button[0], first_button[1])
        if get_registry_value("is_runcard_open", "0"):
            runcard_fill_in_entry(current_runcard_id)
            the_quy_trinh_san_xuat(machine, line, time, wo[1])
    else:
        the_quy_trinh_san_xuat(machine, line, time, 0)
    # print(f"-->{current_runcard_id}")



def runcard_frame_get_runcard_id(machine, line, time):
    def insert_data():
        try:
            sql = """SELECT rc.Id as runcard, wo.Id as workorder
                    FROM [PMGMES].[dbo].[PMG_MES_RunCard] rc
                    JOIN [PMGMES].[dbo].[PMG_MES_WorkOrder] wo
            ...
            with pyodbc.connect(conn_str) as conn:
                cursor = conn.cursor()
                cursor.execute(sql, (machine, line, time))
                result = cursor.fetchall()
                if result:
                    threading.Thread(target=show_error_message, args=(f"Runcard {result[0][0]} có tồn tại", 1, 3000), daemon=True).start()
                    if len(result) > 1:
                        new_result = [[item[0], item[1]] for item in result]
                    else:
                        new_result = [[result[0][0], result[0][1]]]
                    manage_wo_buttons(machine, line, time, new_result)
                else:
                    new_result = []
                    manage_wo_buttons(machine, line, time, new_result)
        except Exception as e:
...
def runcard_time_button_clicked(value, btn, icon, selected_icon):
    """Changes the clicked button's icon to selected and resets the previous one."""
    global selected_time_button, selected_time_icon, selected_time_button_period

    if selected_time_button:
        selected_time_button.config(image=period_button_icons[selected_time_button_period])
    btn.config(image=selected_icon)
    selected_time_button = btn
    selected_time_icon = selected_icon
    selected_time_button_period = value
    runcard_frame_get_runcard_id(selected_machine_button_machine, selected_line_button_line, int(selected_time_button_period))
    # print(f"==> {selected_time_button_period}")
...
for index, period in enumerate(period_times):
    ...
    period_button_icons[period] = icon
    period_selected_icons[period] = icon_selected
    frame_name = f"middle_right_runcard_frame_col3_row{index + 1}"
    col3_frame_dict[frame_name] = tk.Frame(middle_right_runcard_frame_col3, bg='white', width=40, height=26)
    col3_frame_dict[frame_name].place(x=0, y=index * 29, width=40, height=26)
    button = tk.Button(col3_frame_dict[frame_name], image=icon, width=40, height=26, relief="flat")
    button.config(command=lambda p=period: runcard_time_button_clicked(p, period_button_dict[p], period_button_icons[p], period_selected_icons[p]))
    button.place(x=0, y=0, relwidth=1.0, relheight=1.0)
    period_button_dict[period] = button

if current_time in period_button_dict:
    runcard_time_button_clicked(current_time, period_button_dict[current_time], period_button_icons[current_time], period_selected_icons[current_time])
...
def runcard_machine_line_list(plant):
    global machine_line_list
    sql = f"""SELECT DISTINCT(Name) FROM [PMGMES].[dbo].[PMG_DML_DataModelList]
            WHERE DataModelTypeId = 'DMT000003' AND Name LIKE ? 
            ORDER BY Name"""
    conn_str = (
        f'DRIVER={{SQL Server}};'
        f'SERVER={server_ip.get()};'
    ...
    with pyodbc.connect(conn_str) as conn:
        ...
        # print(result)
        if result:
            machine_list = [machine[0] for machine in result]
            return machine_list
        else:
            return []
...
def manage_machine_buttons(parent_frame, plant_name):
    try:
        global selected_machine_button, selected_machine_icon, selected_machine_button_machine
        global machine_button_dict, machine_button_icons, machine_selected_icons

        machine_line_list = runcard_machine_line_list(plant_name)
        machine_button_dict = {}
        machine_button_icons = {}
        machine_selected_icons = {}

        def runcard_machine_button_clicked(value, btn):
            global selected_machine_button, selected_machine_icon, selected_machine_button_machine
            if selected_machine_button:
                if selected_machine_button_machine in machine_button_icons:
                    selected_machine_button.config(image=machine_button_icons[selected_machine_button_machine])

            btn.config(image=machine_selected_icons[value])
            selected_machine_button = btn
            selected_machine_icon = machine_selected_icons[value]
            selected_machine_button_machine = value
            runcard_frame_get_runcard_id(selected_machine_button_machine, selected_line_button_line, int(selected_time_button_period))
            # print(f"==> {selected_machine_button_machine}")

        first_machine = None
        for index, machine in enumerate(machine_line_list):
            normal_icon_path = f"icon/machine/machine_{int(machine[-2:])}.png"
            selected_icon_path = f"icon/machine/machine_{int(machine[-2:])}_hover.png"
            icon = ImageTk.PhotoImage(Image.open(normal_icon_path).resize((40, 26)))
            icon_selected = ImageTk.PhotoImage(Image.open(selected_icon_path).resize((40, 26)))
            machine_button_icons[machine] = icon
            machine_selected_icons[machine] = icon_selected

            frame = tk.Frame(parent_frame, bg='white', width=40, height=26)
            frame.place(x=0, y=index * 29, width=40, height=26)

            button = tk.Button(frame, image=icon, width=40, height=26, relief="flat")
            button.config(command=lambda p=machine, b=button: runcard_machine_button_clicked(p, b))
            button.place(x=0, y=0, relwidth=1.0, relheight=1.0)

            machine_button_dict[machine] = button
            if first_machine is None:
                first_machine = (machine, button)
        if first_machine:
            runcard_machine_button_clicked(first_machine[0], first_machine[1])
    except:
        pass
manage_machine_buttons(middle_right_runcard_frame_col1, get_registry_value('is_plant_name', ""))
...
def manage_line_buttons():
    global selected_line_button, selected_line_icon, selected_line_button_line
    line_list = ['A1', 'B1'] if 'PVC' in str(get_registry_value("is_plant_name", "")).upper() else ['A1', 'B1', 'A2', 'B2']
    middle_width = int(len(line_list) * 48)
    side_width = int((432 - middle_width) / 2)
    middle_right_runcard_frame_col2_row2_left_frame = tk.Frame(middle_right_runcard_frame_col2_row2, bg='white')
    middle_right_runcard_frame_col2_row2_left_frame.place(x=0, y=0, width=side_width, height=40)

    middle_right_runcard_frame_col2_row2_middle_frame = tk.Frame(middle_right_runcard_frame_col2_row2, bg='white')
    middle_right_runcard_frame_col2_row2_middle_frame.place(x=side_width, y=0, width=middle_width, height=40)

    middle_right_runcard_frame_col2_row2_right_frame = tk.Frame(middle_right_runcard_frame_col2_row2, bg='white')
    middle_right_runcard_frame_col2_row2_right_frame.place(x=side_width + middle_width, y=0, width=side_width, height=40)
    line_button_dict = {}
    line_button_icons = {}
    line_selected_icons = {}

    def runcard_line_button_clicked(value, btn):
        global selected_line_button, selected_line_icon, selected_line_button_line
        if selected_line_button:
            selected_line_button.config(image=line_button_icons[selected_line_button_line])
        btn.config(image=line_selected_icons[value])
        selected_line_button = btn
        selected_line_icon = line_selected_icons[value]
        selected_line_button_line = value
        runcard_frame_get_runcard_id(selected_machine_button_machine, selected_line_button_line, int(selected_time_button_period))
        # print(f"==> {selected_line_button_line}")

    first_line = None
    for index, line in enumerate(line_list):
        normal_icon_path = f"icon/line/line_{str(line).lower()}.png"
        selected_icon_path = f"icon/line/line_{str(line).lower()}_hover.png"
        icon = ImageTk.PhotoImage(Image.open(normal_icon_path).resize((40, 26)))
        icon_selected = ImageTk.PhotoImage(Image.open(selected_icon_path).resize((40, 26)))
        line_button_icons[line] = icon
        line_selected_icons[line] = icon_selected
        frame = tk.Frame(middle_right_runcard_frame_col2_row2_middle_frame, bg='white', width=40, height=29)
        frame.place(x=index * 48, y=0, width=40, height=29)
        button = tk.Button(frame, image=icon, width=40, height=26, relief="flat")
        button.config(command=lambda p=line, b=button: runcard_line_button_clicked(p, b))
        button.place(x=0, y=0, relwidth=1.0, relheight=1.0)

        line_button_dict[line] = button
        if first_line is None:
            first_line = (line, button)
    if first_line:
        runcard_line_button_clicked(first_line[0], first_line[1])
manage_line_buttons()
...
top_open_weight_frame_button_icon = ImageTk.PhotoImage(Image.open("icon/weight.png").resize((156, 36)))
top_open_weight_frame_button_hover_icon = ImageTk.PhotoImage(Image.open("icon/weight_hover.png").resize((156, 36)))
...
top_open_thickness_frame_button_icon = ImageTk.PhotoImage(Image.open("icon/thickness.png").resize((156, 36)))
top_open_thickness_frame_button_hover_icon = ImageTk.PhotoImage(Image.open("icon/thickness_hover.png").resize((156, 36)))
...
thickness_entry_delete_icon = ImageTk.PhotoImage(Image.open("icon/delete.png").resize((117, 32)))
thickness_entry_delete_hover_icon = ImageTk.PhotoImage(Image.open("icon/delete_hover.png").resize((117, 32)))
...
weight_entry_delete_icon = ImageTk.PhotoImage(Image.open("icon/delete.png").resize((117, 32)))
weight_entry_delete_hover_icon = ImageTk.PhotoImage(Image.open("icon/delete_hover.png").resize((117, 32)))
...
weight_entry_delete_all_icon = ImageTk.PhotoImage(Image.open("icon/delete_all.png").resize((36, 36)))
weight_entry_delete_all_hover_icon = ImageTk.PhotoImage(Image.open("icon/delete_all_hover.png").resize((36, 36)))
...
thickness_entry_delete_all_icon = ImageTk.PhotoImage(Image.open("icon/delete_all.png").resize((36, 36)))
thickness_entry_delete_all_hover_icon = ImageTk.PhotoImage(Image.open("icon/delete_all_hover.png").resize((36, 36)))
...
setting_icon = ImageTk.PhotoImage(Image.open("icon/setting.png").resize((42, 42)))
setting_hover_icon = ImageTk.PhotoImage(Image.open("icon/setting_hover.png").resize((42, 42)))
...
save_icon = ImageTk.PhotoImage(Image.open("icon/save.png").resize((134, 34)))
save_icon_hover = ImageTk.PhotoImage(Image.open("icon/save_hover.png").resize((134, 34)))
...
close_icon = ImageTk.PhotoImage(Image.open("icon/close.png").resize((134, 34)))
close_icon_hover = ImageTk.PhotoImage(Image.open("icon/close_hover.png").resize((134, 34)))
...
exit_icon = ImageTk.PhotoImage(Image.open("icon/exit.png").resize((134, 31)))
exit_icon_hover = ImageTk.PhotoImage(Image.open("icon/exit_hover.png").resize((134, 31)))
