...
def update_dimensions():
    ...
    while True:
        ...
        if count == 0:
            ...
            count = 1
def embed_browser_in_frame_non_blocking(parent_frame, url, width, height):
    global cef_initialized, browser

    sys.excepthook = cef.ExceptHook

    if not cef_initialized:
        cef.Initialize()
    ...
    window_info.SetAsChild(parent_frame.winfo_id(), [0, 0, width, height])

    browser = cef.CreateBrowserSync(window_info, url=url)

    # Schedule cef.MessageLoopWork periodically
    def loop_cef():
        cef.MessageLoopWork()
        parent_frame.after(10, loop_cef)  # Keep pumping every 10ms

    parent_frame.after(10, loop_cef)
def on_close():
    try:
        cef.QuitMessageLoop()
    except Exception as e:
        print("CEF Shutdown Error:", e)
    finally:
        root.destroy()
...
# Launch browser inside the frame on a separate thread
embed_browser_in_frame_non_blocking(
    parent_frame=middle_right_runcard_frame_row2,
    url="http://10.13.104.181:10000/",
    width=510,
    height=720
)
